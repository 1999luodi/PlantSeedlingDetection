...
class ModelManager(QObject):
    ...
    @staticmethod
    def _read_packaged_model_config(config_file_name):
        """Read packaged auto-labeling model config with Python 3.8 fallback."""
        ...
        # Fallback for Python < 3.9 (no importlib.resources.files)
        pkg_dir = os.path.dirname(auto_labeling_configs.__file__)
        resource_path = os.path.join(
            pkg_dir, "auto_labeling", config_file_name
        )
    ...
    def load_model_configs(self):
        ...
        for model in model_list:
            ...
            if config_file.startswith(":/"):  # Config file is in resources
                ...
                config_content = self._read_packaged_model_config(
                    config_file_name
                )
    ...
    def _load_model(self, model_id):  # noqa: C901
        ...
        if model_config["type"] == "yolov5":
            ...
            try:
                ...
                return
        elif model_config["type"] == "faster_rcnn":
            from .faster_rcnn import FasterRCNN

            try:
                model_config["model"] = FasterRCNN(
                    model_config, on_message=self.new_model_status.emit
                )
                self.auto_segmentation_model_unselected.emit()
                logger.info(
